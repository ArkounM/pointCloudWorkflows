...
eptDirectory = r"E:/_PointCloud/Workflow_6/EPT/"

# Create output tile directory if it does not exist
if not os.path.exists(eptDirectory):
    os.makedirs(eptDirectory)

print("Beginning first step of the pipeline to convert LAZ file to EPT")

# Process each .laz file to ept
for file in os.listdir(lazDirectory):
    if file.endswith(".laz"):
        inputFile = os.path.join(lazDirectory, file)
        outputFolder = os.path.join(eptDirectory, file.split(".")[0])
        ...
        subprocess.run(["entwine", "build", "i", inputFile, "o", outputFolder])

        elapsed_time = time.time() - start_time
        print(f"Completed processing {inputFile} in {elapsed_time:.2f} seconds")

print("All LAZ files turned into EPT successfully")

'''Pipeline 2: EPT to the 3D tiles'''
# Set input and output directories. GZIP directory will need to be set by user 
tileDirectory = r"E:/_PointCloud/Workflow_6/TILE/"
...
for folder in folders:
    inputFolder = os.path.join(eptDirectory, folder)
    outputFolder = os.path.join(tileDirectory, folder)
    ...
    command = ["entwine", "convert", "-i", inputFolder, "-o", outputFolder, "--force"]
...
# Set input and output directories. GZIP directory will need to be set by user 
gzipDirectory = r"E:/_PointCloud/Workflow_6/GZIP"
